...
class HistorialUEMC(db.Model):
    __tablename__ = "historial_uemc"
    id = db.Column(db.Integer, primary_key=True)
    temporada_id = db.Column(
        db.Integer,
        db.ForeignKey("temporadas_uemc.id"),
        nullable=False
    )
    ...
    temporada = db.relationship("TemporadaUEMC")
    
class PalmaresUEMC(db.Model):
    __tablename__ = "palmares_uemc"
    id = db.Column(db.Integer, primary_key=True)
    temporada_id = db.Column(
        db.Integer,
        db.ForeignKey("temporadas_uemc.id"),
    ...
    temporada = db.relationship(
        "TemporadaUEMC",
        backref="palmares_uemc"
    ) 

class TemporadaUEMC(db.Model):
    __tablename__ = "temporadas_uemc"
    id = db.Column(db.Integer, primary_key=True)
...
class JornadaUEMC(db.Model):
    ...
    temporada_id = db.Column(
        db.Integer,
        db.ForeignKey("temporadas_uemc.id", ondelete="CASCADE"),
        nullable=False
    )

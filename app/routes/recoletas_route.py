...
def resultados_recoletas():
    ...
    for i, jornada in enumerate(nuevos_datos_recoletas):
...
def clasif_recoletas():
    ...
    for club in clubs_recoletas:
        if not any(
            ...
            for equipo in clasificacion_analisis_recoletas
        ):
    ...
    clasificacion_analisis_recoletas.sort(
...
# HISTORIAL ATL. VALLADOLID
# Creación del historial de temporadas del Atl. Valladolid
@recoletas_route_bp.route("/admin/crear_historial_recoletas", methods=["GET", "POST"])
def crear_historial_recoletas():
    if request.method == "POST":
        historial = Historial(
            deporte="balonmano",
        ...
        db.session.commit()
        return redirect(url_for("recoletas_route_bp.crear_historial_recoletas"))
    ...
    temporadas = TemporadaRecoletas.query.order_by(
        TemporadaRecoletas.nombre.desc()
    ).all()
    return render_template(
        ...
        deporte="balonmano",
        equipo="Atl. Valladolid",
        crear_url="recoletas_route_bp.crear_historial_recoletas",
        modificar_url="recoletas_route_bp.modificar_historial_recoletas",
        eliminar_url="recoletas_route_bp.eliminar_historial_recoletas"
    )
# Eliminar historial de temporadas del Atl. Valladolid
@recoletas_route_bp.route("/admin/eliminar_historial_recoletas/<int:id>", methods=["POST"])
def eliminar_historial_recoletas(id):
    historial = Historial.query.get_or_404(id)
    db.session.delete(historial)
    db.session.commit()
    return redirect(url_for("recoletas_route_bp.crear_recoletas_aula"))
# Modificar historial de temporadas del Atl. Valladolid
@recoletas_route_bp.route("/admin/modificar_historial_recoletas/<int:id>", methods=["POST"])
def modificar_historial_recoletas(id):
    historial = Historial.query.get_or_404(id)
    ...
    db.session.commit()
    return redirect(url_for("recoletas_route_bp.crear_historial_recoletas"))
# Ver Historial de temporadas del Atl. Valladolid en la página principal
@recoletas_route_bp.route("/recoletas/historial")
def historial_recoletas():
    historial = (Historial.query.filter_by(
        deporte="balonmano",
        equipo="Atl. Valladolid"
    ).order_by(Historial.temporada.desc()).all())
    ...
    # GRÁFICO JORNADAS
    temporadas = TemporadaRecoletas.query.order_by(TemporadaRecoletas.id).all()
    datasets_jornadas = []
    colores = [
        "#672e8d",
        "#FFD700",
    ...
    for i, temporada in enumerate(temporadas):
        jornadas = (
            JornadaRecoletas.query.filter_by(temporada_id=temporada.id)
            .order_by(JornadaRecoletas.id)
            .all()
        )
        if not jornadas:
            continue
        labels, puntos = obtener_evolucion_puntos(
            jornadas, "Atl. Valladolid", generar_clasificacion_analisis_balonmano_recoletas,"puntos"
        )
    ...
    return render_template(
        "historia/historia_recoletas.html",
        historial=historial,
...
# Crear Palmares del Atl. Valladolid
@recoletas_route_bp.route("/admin/crear_palmares_recoletas", methods=["GET", "POST"])
def crear_palmares_recoletas():
    if request.method == "POST":
        titulo = Palmaress(
            deporte="balonmano",
            equipo="Atl. Valladolid",
    ...
    return render_template(
        ...
        deporte="Balonmano",
        equipo="Atl. Valladolid",
        crear_url="recoletas_route_bp.crear_palmares_recoletas",
        modificar_url="recoletas_route_bp.modificar_palmares_recoletas",
        eliminar_url="recoletas_route_bp.eliminar_palmares_recoletas",
    )
# Modificar Palmares del Atl. Valladolid
@recoletas_route_bp.route("/admin/modificar_palmares_recoletas/<int:id>", methods=["POST"])
def modificar_palmares_recoletas(id):
    ...
    db.session.commit()
    return redirect(url_for("recoletas_route_bp.crear_palmares_recoletas"))
# Eliminar Palmares del Atl. Valladolid
@recoletas_route_bp.route("/admin/eliminar_palmares_recoletas/<int:id>", methods=["POST"])
def eliminar_palmares_recoletas(id):
    titulo = Palmaress.query.get_or_404(id)

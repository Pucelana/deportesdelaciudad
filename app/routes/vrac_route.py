...
def resultados_vrac():
    ...
    for i, jornada in enumerate(nuevos_datos_vrac):
...
# HISTORIAL VRAC
# Creación del historial de temporadas del VRAC
@vrac_route_bp.route("/admin/crear_historial_vrac", methods=["GET", "POST"])
def crear_historial_vrac():
    if request.method == "POST":
        historial = Historial(
            deporte="rugby",
            equipo="VRAC",
        ...
        db.session.commit()
        return redirect(url_for("vrac_route_bp.crear_historial_vrac"))
    ...
    return render_template(
        ...
        deporte="rugby",
        equipo="VRAC",
        crear_url="vrac_route_bp.crear_historial_vrac",
        modificar_url="vrac_route_bp.modificar_historial_vrac",
        eliminar_url="vrac_route_bp.eliminar_historial_vrac"
    )
# Eliminar historial de temporadas del VRAC
@vrac_route_bp.route("/admin/eliminar_historial_vrac/<int:id>", methods=["POST"])
def eliminar_historial_vrac(id):
    historial = Historial.query.get_or_404(id)
    db.session.delete(historial)
    db.session.commit()
    return redirect(url_for("vrac_route_bp.crear_historial_vrac"))
# Modificar historial de temporadas del VRAC
@vrac_route_bp.route("/admin/modificar_historial_vrac/<int:id>", methods=["POST"])
def modificar_historial_vrac(id):
    historial = Historial.query.get_or_404(id)
    ...
    db.session.commit()
    return redirect(url_for("vrac_route_bp.crear_historial_vrac"))
# Ver Historial de temporadas del VRAC en la página principal
@vrac_route_bp.route("/vrac/historial")
def historial_vrac():
    historial = (Historial.query.filter_by(
        deporte="rugby",
        equipo="VRAC"
    ...
    # GRÁFICO JORNADAS
    temporadas = TemporadaVrac.query.order_by(TemporadaVrac.id).all()
    datasets_jornadas = []
    colores = [
        "#672e8d",
        "#FFD700",
    ...
    for i, temporada in enumerate(temporadas):
        jornadas = (
            JornadaVrac.query.filter_by(temporada_id=temporada.id)
            .order_by(JornadaVrac.id)
            .all()
        )
        if not jornadas:
            continue
        labels, puntos = obtener_evolucion_puntos(
            jornadas, "VRAC", generar_clasificacion_analisis_rugby_vrac,"puntos"
        )
...
@vrac_route_bp.route("/admin/crear_palmares_vrac", methods=["GET", "POST"])
def crear_palmares_vrac():
    if request.method == "POST":
        titulo = Palmaress(
            deporte="rugby",
            equipo="VRAC",
        ...
        db.session.commit()
        return redirect(url_for("vrac_route_bp.crear_palmares_vrac"))
    palmares = (
    ...
    return render_template(
        ...
        deporte="Rugby",
        equipo="VRAC",
        crear_url="vrac_route_bp.crear_palmares_vrac",
        modificar_url="vrac_route_bp.modificar_palmares_vrac",
        eliminar_url="vrac_route_bp.eliminar_palmares_vrac",
    )
# Modificar Palmares del VRAC
@vrac_route_bp.route("/admin/modificar_palmares_vrac/<int:id>", methods=["POST"])
def modificar_palmares_vrac(id):
    ...
    db.session.commit()
    return redirect(url_for("vrac_route_bp.crear_palmares_vrac"))
# Eliminar Palmares del VRAC
@vrac_route_bp.route("/admin/eliminar_palmares_vrac/<int:id>", methods=["POST"])
def eliminar_palmares_vrac(id):
    titulo = Palmaress.query.get_or_404(id)

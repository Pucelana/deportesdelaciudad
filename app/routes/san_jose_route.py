...
@san_jose_route_bp.route("/admin/crear_historial_san_jose", methods=["GET", "POST"])
def crear_historial_san_jose():
    if request.method == "POST":
        historial = Historial(
            deporte="voley",
            equipo="CD San Jose",
...
def eliminar_historial_san_jose(id):
    historial = Historial.query.get_or_404(id)
    db.session.delete(historial)
    db.session.commit()
    return redirect(url_for("san_jose_route_bp.crear_historial_san_jose"))
# Modificar historial de temporadas del CD San Jose
@san_jose_route_bp.route("/admin/modificar_historial_san_jose/<int:id>", methods=["POST"])
def modificar_historial_san_jose(id):
    historial = Historial.query.get_or_404(id)
...
def historial_san_jose():
    ...
    # GRÁFICO JORNADAS
    temporadas = TemporadaJose.query.order_by(TemporadaJose.id).all()
    datasets_jornadas = []
    colores = [
        "#672e8d",
        "#FFD700",
    ...
    return render_template(
        "historia/historia_san_jose.html",
        historial=historial,
...
@san_jose_route_bp.route("/admin/crear_palmares_san_jose", methods=["GET", "POST"])
def crear_palmares_san_jose():
    if request.method == "POST":
        titulo = Palmaress(
            deporte="voley",
            equipo="CD San Jose",
...
# Modificar Palmares del CD San Jose
@san_jose_route_bp.route("/admin/modificar_palmares_san_jose/<int:id>", methods=["POST"])
def modificar_palmares_san_jose(id):
    ...
    db.session.commit()
    return redirect(url_for("san_jose_route_bp.crear_palmares_san_jose"))
# Eliminar Palmares del CD San Jose
@san_jose_route_bp.route("/admin/eliminar_palmares_san_jose/<int:id>", methods=["POST"])
def eliminar_palmares_san_jose(id):
    titulo = Palmaress.query.get_or_404(id)

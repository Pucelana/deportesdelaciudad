from app.extensions import db

class HistorialParquesol(db.Model):
    __tablename__ = "historial_parquesol"
    id = db.Column(db.Integer, primary_key=True)
    temporada_id = db.Column(
        db.Integer,
        db.ForeignKey("temporadas_parquesol.id"),
        nullable=False
    )
    liga = db.Column(db.String(80), nullable=False)
    puntos = db.Column(db.Integer)
    puesto = db.Column(db.Integer)
    playoff = db.Column(db.String(80))
    copa = db.Column(db.String(80))
    titulos = db.Column(db.String(150))
    siguiente_temporada = db.Column(db.String(80))
    observaciones = db.Column(db.String(200))
    temporada = db.relationship("TemporadaParquesol")
    
class PalmaresParquesol(db.Model):
    __tablename__ = "palmares_parquesol"
    id = db.Column(db.Integer, primary_key=True)
    temporada_id = db.Column(
        db.Integer,
        db.ForeignKey("temporadas_parquesol.id"),
        nullable=False
    )
    competicion = db.Column(db.String(100), nullable=False)
    imagen = db.Column(db.String(100), nullable=False)
    temporada = db.relationship(
        "TemporadaParquesol",
        backref="palmares_parquesol"
    ) 


class TemporadaParquesol(db.Model):
    __tablename__ = "temporadas_parquesol"
    id = db.Column(db.Integer, primary_key=True)
    nombre = db.Column(db.String(20), unique=True, nullable=False)
    activa = db.Column(db.Boolean, default=False)
    jornadas = db.relationship(
        "JornadaParquesol",
        backref="temporada",
        cascade="all, delete-orphan"
    )
    
class JornadaParquesol(db.Model):
    __tablename__ = "jornadas_parquesol"
    id = db.Column(db.Integer, primary_key=True)
    temporada_id = db.Column(
        db.Integer,
        db.ForeignKey("temporadas_parquesol.id", ondelete="CASCADE"),
        nullable=False
    )
    nombre = db.Column(db.String(255), nullable=False)
    partidos = db.relationship(
        "ParquesolPartido",
        backref="jornada",
        cascade="all, delete-orphan"
    ) 
    
class ParquesolPartido(db.Model):
    __tablename__ = 'parquesol_partidos'
    id = db.Column(db.Integer, primary_key=True)
    jornada_id = db.Column(db.Integer, db.ForeignKey('jornadas_parquesol.id'), nullable=False)
    fecha = db.Column(db.String(25))
    hora = db.Column(db.String(25))
    local = db.Column(db.String(255))
    resultadoA = db.Column(db.String(120))
    resultadoB = db.Column(db.String(120))
    visitante = db.Column(db.String(255))
    orden = db.Column(db.Integer)   

class ParquesolClub(db.Model):
    __tablename__ = 'parquesol_clubs'
    # Definir la columna ID
    id = db.Column(db.Integer, primary_key=True)
    # Definir la columna 'nombre' para el nombre del club
    nombre = db.Column(db.String(255), nullable=False)

class CopaParquesol(db.Model):
    __tablename__ = 'copa_parquesol'
    id = db.Column(db.Integer, primary_key=True)  # ID único para cada partido
    eliminatoria = db.Column(db.String(50), nullable=False)
    fecha = db.Column(db.String(20))  # Fecha del partido
    hora = db.Column(db.String(20))  # Hora del partido
    local = db.Column(db.String(255))  # Nombre del equipo local
    resultadoA = db.Column(db.String(120))  # Resultado del equipo local
    resultadoB = db.Column(db.String(120))  # Resultado del equipo visitante
    visitante = db.Column(db.String(255))
    
class PlayoffParquesol(db.Model):
    __tablename__ = 'playoff_parquesol'
    id = db.Column(db.Integer, primary_key=True)
    eliminatoria = db.Column(db.String(20), nullable=False)
    fecha = db.Column(db.String(20), nullable=True)
    hora = db.Column(db.String(10), nullable=True)
    local = db.Column(db.String(100), nullable=True)
    resultadoA = db.Column(db.String(10), nullable=True)
    resultadoB = db.Column(db.String(10), nullable=True)
    visitante = db.Column(db.String(100), nullable=True)
    orden = db.Column(db.Integer) 
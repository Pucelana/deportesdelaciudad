...
def ingresar_resultado_cdsi_vall():
    if request.method == 'POST':
        ...
        temporada = TemporadaCDSIVall.query.filter_by(nombre=temporada_nombre).first()
        if not temporada:
            temporada = TemporadaCDSIVall(nombre=temporada_nombre, activa=False)
            db.session.add(temporada)
            db.session.flush()
        # 2. crear jornada correcta
        jornada = JornadaCDSIVall(
...
def calendarios_cdsi_vall():
    ...
    if temporada:
        jornadas = JornadaCDSIVall.query.filter_by(
        temporada_id=temporada.id
        ).order_by(JornadaCDSIVall.id.asc()).all()
    else:
        jornadas = []
...
def obtener_datos_cdsi_vall(nombre_temporada=None):
    if nombre_temporada is None:
        temporada = TemporadaCDSIVall.query.filter_by(activa=True).first()
    else:
        temporada = TemporadaCDSIVall.query.filter_by(nombre=nombre_temporada).first()
    if not temporada:
        return []
    jornadas_con_partidos = []
    for jornada in temporada.jornadas:
        partidos = (
            CDSIVallPartido.query
            .filter_by(jornada_id=jornada.id)
            .order_by(CDSIVallPartido.orden.asc())
            .all()
        )
...
def temporadas_cdsi_vall():
    ...
    return render_template(
        'admin/temporadas/temporada_cdsi_vall.html',
        temporadas=temporadas
    )
# ACTIVAR Y DESACTIVAR TEMPORADAS
@cdsi_vall_route_bp.route('/activar_temporada_cdsi_vall/<int:id>')
def activar_temporada_cdsi_vall(id):
    TemporadaCDSIVall.query.update({"activa": False})
    temporada = TemporadaCDSIVall.query.get_or_404(id)
    temporada.activa = True
    db.session.commit()
    return redirect(url_for('cdsi_vall_route_bp.temporadas_cdsi_vall'))   
    
# HISTORIAL CDSI VALLADOLID
# Creación del historial de temporadas del Ponce
@cdsi_vall_route_bp.route("/admin/crear_historial_cdsi_vall", methods=["GET", "POST"])
def crear_historial_cdsi_vall():
    if request.method == "POST":
        historial = Historial(
            deporte="baloncesto",
    ...
    return render_template(
        ...
        deporte="baloncesto",
        equipo="CDSI Valladolid",
        crear_url="cdsi_vall_route_bp.crear_historial_cdsi_vall",
        modificar_url="cdsi_vall_route_bp.modificar_historial_cdsi_vall",
        eliminar_url="cdsi_vall_route_bp.eliminar_historial_cdsi_vall"
    )
@cdsi_vall_route_bp.route("/admin/eliminar_historial_cdsi_vall/<int:id>", methods=["POST"])
def eliminar_historial_cdsi_vall(id):
    historial = Historial.query.get_or_404(id)
    db.session.delete(historial)
    db.session.commit()
    return redirect(url_for("cdsi_vall_route_bp.crear_historial_cdsi_vall"))
# Modificar historial de temporadas del CDSI Valladolid
@cdsi_vall_route_bp.route("/admin/modificar_historial_cdsi_vall/<int:id>", methods=["POST"])
def modificar_historial_cdsi_vall(id):
    historial = Historial.query.get_or_404(id)
...
def historial_cdsi_vall():
    ...
    # GRÁFICO JORNADAS
    temporadas = TemporadaCDSIVall.query.order_by(TemporadaCDSIVall.id).all()
    datasets_jornadas = []
    colores = [
        "#672e8d",
        "#FFD700",
    ...
    for i, temporada in enumerate(temporadas):
        jornadas = (
            JornadaCDSIVall.query.filter_by(temporada_id=temporada.id)
            .order_by(JornadaCDSIVall.id)
            .all()
        )
        if not jornadas:
            continue
        ...
        labels, puntos = obtener_evolucion_puntos(
            jornadas, "CDSI Valladolid", generar_clasificacion_analisis_baloncesto_cdsi_vall,"ganados"
        )
...
@cdsi_vall_route_bp.route("/admin/crear_palmares_cdsi_vall", methods=["GET", "POST"])
def crear_palmares_cdsi_vall():
    if request.method == "POST":
        titulo = Palmaress(
            deporte="baloncesto",
            equipo="CDSI Valladolid",
    ...
    return render_template(
        ...
        deporte="baloncesto",
        equipo="CDSI Valladolid",
        crear_url="cdsi_vall_route_bp.crear_palmares_cdsi_vall",
        modificar_url="cdsi_vall_route_bp.modificar_palmares_cdsi_vall",
        eliminar_url="cdsi_vall_route_bp.eliminar_palmares_cdsi_vall",
    )
# Modificar Palmares del CDSI VALLADOLID
@cdsi_vall_route_bp.route("/admin/modificar_palmares_cdsi_vall/<int:id>", methods=["POST"])
def modificar_palmares_cdsi_vall(id):
...
@cdsi_vall_route_bp.route("/admin/eliminar_palmares_cdsi_vall/<int:id>", methods=["POST"])
def eliminar_palmares_cdsi_vall(id):
    titulo = Palmaress.query.get_or_404(id)

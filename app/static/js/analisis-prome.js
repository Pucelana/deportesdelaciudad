...
/*function calcularEstadisticas() {
    const tablaDatos = document.getElementById('tablaClasifPromesas');
...
const filas = document.querySelectorAll("#tablaAscensoPromesas tbody tr");
const partidosTotales = 34; // Cambiado a 42 partidos en la temporada
const puntosPorGanar = 3; // Cambiado a 3 puntos por partido ganado
const proximidadFija = 89; // Ajusta este valor según tus necesidades
const equipos = [];
let index = 1;
filas.forEach((fila) => {
    ...
    const partidosGanadosPesimistas = Math.min(partidosGanadosMatematicos - 6, puntosParaAscenso);
    const partidosGanadosOptimistas = Math.min(partidosGanadosMatematicos -10, puntosParaAscenso);
    equipos.push({
        index: index,
        equipo,
        partidosJugados,
...
const tabla = document.querySelector("#tablaAscensoPromesas tbody");
...
equipos.forEach((equipoData) => {
    ...
    equipoData.equipo.includes("RV Promesas") ? "equipo-pucela" : ""}">${equipoData.equipo}</td>
    ...
    <td class="pts-mate1 equipo-mobile text-center">${equipoData.partidosGanadosMatematicos}</td>
...
const filas1 = document.querySelectorAll("#tablaPlayPromesas tbody tr");
const partidosTotales1 = 34; // Cambiado a 42 partidos en la temporada
...
filas1.forEach((fila) => {
    ...
    const partidosGanadosOptimistas1 = Math.min(partidosGanadosMatematicos1 -6, puntosParaAscenso1);
    equipos1.push({
        index1 : index1,
        equipo1,
        partidosJugados1,
...
const tabla1 = document.querySelector("#tablaPlayPromesas tbody");
...
equipos1.forEach((equipo1Data) => {
    ...
    } else if (equipo1Data.index1 <= 5) {
        claseColor1 = 'pos-playoff';
    } else if (equipo1Data.index1 <= 18) {
        claseColor1 = 'pos-nada';
    }
    nuevaFila1.innerHTML = `
    <td class="equipo-mobile text-center ${claseColor1}">${equipo1Data.index1}</td>
    <td class="equipo-mobile text-start size_equipos2 ${
    equipo1Data.equipo1.includes("RV Promesas") ? "equipo-pucela" : ""}">${equipo1Data.equipo1}</td>
...
const filas2 = document.querySelectorAll("#tablaDescPromesas tbody tr");
const partidosTotales2 = 34; // Cambiado a 42 partidos en la temporada
const puntosPorGanar2 = 3; // Cambiado a 3 puntos por partido ganado
const proximidadFijar2 = 62; // Ajusta este valor según tus necesidades
const equipos2 = [];
let index2 = 1;
filas2.forEach((fila, indice) => {
    ...
    const partidosGanadosOptimistas2 = Math.min(partidosGanadosMatematicos2 -17, puntosPermanencia2);
    equipos2.push({
        index2: index2,
        equipo2,
        partidosJugados2,
...
const tabla2 = document.querySelector("#tablaDescPromesas tbody");
...
equipos2.forEach((equipo2Data) => {
    ...
    <td class="desc-mate equipo-mobile text-center">${equipo2Data.partidosGanadosMatematicos2}</td>

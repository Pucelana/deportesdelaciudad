...
def ingresar_resultado_uemc():
    if request.method == "POST":
        ...
        jornada = JornadaUEMC(nombre=nombre_jornada, temporada_id=temporada.id)
    ...
    if jornada:
        if request.method == "POST":
            ...
            for i in range(num_partidos):
                ...
                partido = (
                    db.session.query(UEMCPartido)
                    .filter(UEMCPartido.id == partido_id)
                    .first()
                )
                if partido:
                    partido.fecha = fecha
...
def obtener_datos_uemc(nombre_temporada=None):
    ...
    for jornada in temporada.jornadas:
        partidos = (
            UEMCPartido.query.filter_by(jornada_id=jornada.id)
            .order_by(UEMCPartido.orden.asc())
            .all()
        )
        ...
        for partido in jornada["partidos"]:
            ...
            if equipo_local == equipo_uemc or equipo_visitante == equipo_uemc:
                ...
                if "primer_enfrentamiento" not in tabla_partidos_uemc[equipo_contrario]:
                    tabla_partidos_uemc[equipo_contrario]["primer_enfrentamiento"] = (
                        jornada["nombre"]
                    )
                    ...
                    not in tabla_partidos_uemc[equipo_contrario]
                ):
                    tabla_partidos_uemc[equipo_contrario]["segundo_enfrentamiento"] = (
                        jornada["nombre"]
                    )
                ...
                if (
                    jornada["nombre"]
                    not in tabla_partidos_uemc[equipo_contrario]["jornadas"]
                ):
                    tabla_partidos_uemc[equipo_contrario]["jornadas"][
                        jornada["nombre"]
                    ] = {
                ...
                if (
                    ...
                    if not tabla_partidos_uemc[equipo_contrario]["jornadas"][
                        jornada["nombre"]
                    ]["resultadoA"]:
                        tabla_partidos_uemc[equipo_contrario]["jornadas"][
                            jornada["nombre"]
...
def generar_clasificacion_analisis_baloncesto_uemc(data):
    ...
    def clasificacion_particular(equipos_empatados):
        ...
        for partido in enfrentamientos:
            ...
            if local not in equipos_empatados or visitante not in equipos_empatados:
                continue
    ...
    def comparar_equipos(a, b):
        ...
        if datos_a["diferencia_canastas"] != datos_b["diferencia_canastas"]:

            return datos_b["diferencia_canastas"] - datos_a["diferencia_canastas"]
    ...
    equipos.sort(key=cmp_to_key(comparar_equipos))
...
def clasif_analisis_uemc():
    ...
    for club in clubs_uemc:
        if not any(
            equipo["equipo"] == club.nombre for equipo in clasificacion_analisis_uemc
        ):
...
def crear_historial_uemc():
    ...
    historial = (
        Historial.query.filter_by(deporte="baloncesto", equipo="CBC Valladolid")
        .order_by(Historial.temporada.desc())
        .all()
    )
    temporadas = TemporadaUEMC.query.order_by(TemporadaUEMC.nombre.desc()).all()
    return render_template(
        ...
        deporte="baloncesto",
        equipo="CBC Valladolid",
        crear_url="uemc_route_bp.crear_historial_uemc",
...
def historial_uemc():
    ...
    titulos = (
        Palmaress.query.filter_by(deporte="baloncesto", equipo="CBC Valladolid")
    ...
    for i, temporada in enumerate(temporadas):
        ...
        labels, puntos = obtener_evolucion_puntos(
            jornadas,
            "CBC Valladolid",
            generar_clasificacion_analisis_baloncesto_uemc,
            "ganados",
        )
...
@uemc_route_bp.route("/admin/crear_palmares_uemc", methods=["GET", "POST"])
def crear_palmares_uemc():
    if request.method == "POST":
        ...
        db.session.commit()
        return redirect(url_for("uemc_route_bp.crear_palmares_uemc"))
    palmares = (
        Palmaress.query.filter_by(deporte="baloncesto", equipo="CBC Valladolid")
        .order_by(Palmaress.orden.asc(), Palmaress.temporada.desc())
        .all()
    ...
    return render_template(
        ...
        deporte="baloncesto",
        equipo="CBC Valladolid",
        crear_url="uemc_route_bp.crear_palmares_uemc",
...
def modificar_equipo_copa_uemc(id):
...
def eliminar_equipo_copa_uemc(id):
    equipo = EquiposCopaUEMC.query.get_or_404(id)
...
def ver_copa_uemc():
    ...
    datos_jornadas = {
        j: CopaUEMC.query.filter_by(encuentros=j).order_by(CopaUEMC.id).all()
        for j in jornadas
    }
    # Diccionario para eliminatorias
    datos_eliminatorias = {
        f: CopaUEMC.query.filter_by(encuentros=f).order_by(CopaUEMC.id).all()
        for f in fases_eliminatorias
    }
    equipos = EquiposCopaUEMC.query.order_by(
        EquiposCopaUEMC.grupo, EquiposCopaUEMC.equipo
    ).all()
    return render_template(
        "admin/copa/copa_uemc.html",
        datos_jornadas=datos_jornadas,
        datos_eliminatorias=datos_eliminatorias,
        equipos=equipos,
    )
...
def actualizar_clasificacion(local, resultado_local, resultado_visitante, visitante):
    ...
    resultado_visitante = (
        int(resultado_visitante) if resultado_visitante.isdigit() else None
    )
    ...
    if not clasificacion_local:
        clasificacion_local = Clasificacion(
            equipo=local, jugados=0, ganados=0, perdidos=0, puntos=0, pf=0, pc=0
        )
        db.session.add(clasificacion_local)
    # Si el visitante no existe, lo creamos
    if not clasificacion_visitante:
        clasificacion_visitante = Clasificacion(
            equipo=visitante, jugados=0, ganados=0, perdidos=0, puntos=0, pf=0, pc=0
        )
...
def obtener_jornadas_liga(grupo=None):
    consulta = CopaUEMC.query.filter(CopaUEMC.encuentros.like("liga_%"))
    if grupo:
        consulta = consulta.filter_by(grupo=grupo)
    partidos = consulta.order_by(CopaUEMC.orden, CopaUEMC.id).all()
    jornadas = {}
    for partido in partidos:
        jornada = partido.encuentros
...
def obtener_eliminatorias():
    ...
    partidos = (
        CopaUEMC.query.filter(CopaUEMC.encuentros.in_(fases))
        .order_by(CopaUEMC.orden)
        .all()
    )
...
def recalcular_clasificacion(jornadas):
    clasificacion = {}
    # Cargar todos los equipos antes de procesar partidos
    equipos = EquiposCopaUEMC.query.order_by(
        EquiposCopaUEMC.grupo, EquiposCopaUEMC.equipo
    ).all()
    for e in equipos:
        clasificacion[e.equipo] = {
            "grupo": e.grupo,
            "equipo": e.equipo,
            "jugados": 0,
            "ganados": 0,
...
def obtener_clasificaciones_grupos():
    clasificaciones = {}
    for grupo in ["A", "B", "C", "D", "E", "F", "G"]:
        # Equipos del grupo
        equipos = (
            EquiposCopaUEMC.query.filter_by(grupo=grupo)
            .order_by(EquiposCopaUEMC.equipo)
            .all()
        )
        ...
        partidos = CopaUEMC.query.filter(
            CopaUEMC.grupo == grupo, CopaUEMC.encuentros.like("liga_%")
        ).all()
        for partido in partidos:
            if partido.resultadoA is None or partido.resultadoB is None:
                continue
            local = tabla[partido.local]
            visitante = tabla[partido.visitante]
            local["jugados"] += 1
            visitante["jugados"] += 1
            local["favor"] += partido.resultadoA
            local["contra"] += partido.resultadoB
            visitante["favor"] += partido.resultadoB
            visitante["contra"] += partido.resultadoA
            if partido.resultadoA > partido.resultadoB:
                local["ganados"] += 1
                local["puntos"] += 2
    ...
    try:
        ...
        for i in range(num_partidos):
            ...
            if partido:
                ...
                partido.resultadoA = safe_int(request.form.get(f"resultadoA{i}"))
                partido.resultadoB = safe_int(request.form.get(f"resultadoB{i}"))
...
def uemc_copa():
    ...
    clasificaciones_grupos = obtener_clasificaciones_grupos()
...
def crear_playoff_uemc():
    if request.method == "POST":
        ...
        for i in range(num_partidos):
            ...
            resultadoA = (
                int(resultadoA_raw)
                if resultadoA_raw not in ("", "None", None)
                else None
            )
            resultadoB = (
                int(resultadoB_raw)
                if resultadoB_raw not in ("", "None", None)
                else None
            )
            partido = PlayoffUEMC(
...
def ver_playoff_uemc():
    ...
    for eliminatoria in eliminatorias:
        partidos = (
            PlayoffUEMC.query.filter_by(eliminatoria=eliminatoria)
            .order_by(PlayoffUEMC.orden)
            .all()
        )
